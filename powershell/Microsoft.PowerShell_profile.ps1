...
function Test-InteractiveTerminal {
    ...
    # 2) Вывод не должен быть редиректнут (PSReadLine этого не любит)
    try {
        if ([Console]::IsOutputRedirected -or [Console]::IsErrorRedirected) { return $false }
    } catch { return $false }

    # 3) TERM=dumb или пустой TERM часто означает "без возможностей"
    if ($env:TERM -eq "dumb") { return $false }

    # 4) Проверка на VT/ANSI: пытаемся включить VirtualTerminalProcessing на Windows
    if ($IsWindows) {
        try {
...
"@ -ErrorAction SilentlyContinue | Out-Null
            ...
            $mode = 0
            if (-not [Win32.Native]::GetConsoleMode($h, [ref]$mode)) { return $false }

            # Пробуем включить VT (если нельзя — значит среда не поддерживает)
            if (-not ($mode -band $ENABLE_VIRTUAL_TERMINAL_PROCESSING)) {
                if (-not [Win32.Native]::SetConsoleMode($h, $mode -bor $ENABLE_VIRTUAL_TERMINAL_PROCESSING)) {
                    return $false
                }
            }
    ...
    # На *nix обычно достаточно не-dumb + не-redirected
    return $true
}

$script:IsRichTerminal = Test-InteractiveTerminal

if ($env:TERM_PROGRAM -match 'vscode|cursor' -or $env:CURSOR_EDITOR) {
    $script:IsRichTerminal = $false
}
